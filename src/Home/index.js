...
const Home = () => {
  ...
  const renderTabMenuList = () =>
    response.map(eachCategory => {
      ...
      return (
        <li
          className={`eachitem ${
            eachCategory.menuCategoryId === activeCategoryId ? 'item' : ''
          }`}
          key={eachCategory.menuCategoryId}
          onClick={onClickHandler}
        >
          <button type="button" className="mt-0 mb-0 ms-2 me-2 button">
  ...
  const renderDishes = () => {
    ...
    return (
      <ul className="m-0 d-flex flex-column listcontainer">
  ...
  return isLoading ? (
    ...
    <div className="home-background">
      ...
      <ul className="m-0 ps-0 d-flex container2">{renderTabMenuList()}</ul>
      {renderDishes()}
    </div>
  )
}
